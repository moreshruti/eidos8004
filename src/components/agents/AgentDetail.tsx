...
function usageTypeBadgeClass(): string {
  return 'border border-c3 text-c7'
}
...
export default function AgentDetail({ agent, attributions, designTitles }: AgentDetailProps) {
  ...
  const avgPerAttribution =
    attributions.length > 0 ? totalPaid / attributions.length : 0
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        {/* Agent Card */}
        <div className="border border-c3 p-4">
          <p className="text-c5 font-mono uppercase tracking-[0.2em] text-[10px] mb-1">Agent Card</p>
          <a
            href={`https://ipfs.io/ipfs/${agent.agentCardURI.replace('ipfs://', '')}`}
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-sm text-c9 hover:text-c11 flex items-center gap-1 transition-colors"
          >
            {truncateAddress(agent.agentCardURI)}
      ...
      {/* Capabilities */}
      <div className="mb-8">
        <h2 className="text-c5 font-mono uppercase tracking-[0.2em] text-[10px] mb-3">Capabilities</h2>
